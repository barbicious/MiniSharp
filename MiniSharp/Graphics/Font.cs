...
public static class Font
{
    ...
    {
        for (int cy = 0; cy < CharRows; cy++)
        {
            for (int cx = 0; cx < CharColumns; cx++)
            {
                ...
                renderer.BlitSprite(new SpriteOrder { Colors = [foreground, 0, 0, background], Src = new Rectangle(cx * CharWidth, cy * CharHeight, CharWidth, CharHeight), Dst = new Point(x, y), TextureId = FontId } );
            }
        }
    }
